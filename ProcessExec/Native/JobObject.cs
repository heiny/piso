...
namespace ProcessExec.Native
{
    ...
    public class JobObject : IDisposable
    {
        ...
        /// <summary>
        /// The percent CPU rate limit for all processes in the job [1-100]
        /// </summary>
        private byte jobCpuRateLimit = 0;
        ...
        /// <summary>
        /// Gets or sets the CPU rate limit (percent from 1-100). Set to 0 (zero) to disable the limit.
        /// </summary>
        /// <value>
        /// The job cpu limit.
        /// </value>
        public byte JobCpuLimit
        {
            get
            {
                return this.jobCpuRateLimit;
            }

            set
            {
                this.jobCpuRateLimit = value;
                this.UpdateCpuRateControl();
            }
        }
        ...
        private void UpdateCpuRateControl()
        {
            if (jobCpuRateLimit > 0)
            {
                ...
                try
                {
                    Marshal.StructureToPtr(cpuRateControlInfo, ptr, false);

                    if(!NativeMethods.SetInformationJobObject(jobHandle, NativeMethods.JobObjectInfoClass.JobObjectCpuRateControlInformation, ptr, (uint)len))
                    {
                ...
                {
                    throw new Exception("Error updating cpu rate control", ex);
                }
                finally
                {
