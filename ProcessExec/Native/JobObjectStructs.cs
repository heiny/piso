...
namespace ProcessExec.Native
{
    internal partial class NativeMethods
    {
        ...
        /// </summary>
        /// <reference>http://msdn.microsoft.com/en-us/library/windows/desktop/hh448384(v=vs.85).aspx</reference>
        [StructLayout(LayoutKind.Sequential)]
        public struct JOBOBJECT_CPU_RATE_CONTROL_INFORMATION
        {
            public const uint JOB_OBJECT_CPU_RATE_CONTROL_ENABLE = 0x00000001;
            public const uint JOB_OBJECT_CPU_RATE_CONTROL_WEIGHT_BASED = 0x00000002;
            public const uint JOB_OBJECT_CPU_RATE_CONTROL_HARD_CAP = 0x00000004;
            public const uint JOB_OBJECT_CPU_RATE_CONTROL_NOTIFY = 0x00000008;

            public static JOBOBJECT_CPU_RATE_CONTROL_INFORMATION RateLimited(Byte percentCpuLimit)
            {
                if (percentCpuLimit < 1 || percentCpuLimit > 100)
                {
                    throw new ArgumentOutOfRangeException("percentCpuLimit", "Expected percentage: 1 <= value <= 100");
                }

                return new JOBOBJECT_CPU_RATE_CONTROL_INFORMATION
                {
                    ControlFlags = JOB_OBJECT_CPU_RATE_CONTROL_ENABLE | JOB_OBJECT_CPU_RATE_CONTROL_HARD_CAP,
                    CpuRateWeight = new CpuRateWeightUnion
                    {
                        CpuRate = (uint) percentCpuLimit*100
                    }
                };
            }
            ...
            /// CPU Rate / Weight
            /// </summary>
            public CpuRateWeightUnion CpuRateWeight;
        }

        [StructLayout(LayoutKind.Explicit)]
        public struct CpuRateWeightUnion
        {
            /// <summary>
            /// If the ControlFlags member specifies JOB_OBJECT_CPU_RATE_WEIGHT_BASED, this member is not used.
            /// </summary>
            [FieldOffset(0)]
            public uint CpuRate;
